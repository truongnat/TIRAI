...
export async function runExecute(opts: ExecuteOptions): Promise<void> {
  ...
  if (platform === 'backend' || platform === 'database') {
    ...
    fs.writeFileSync(planPath, JSON.stringify({
      schemaVersion: '1.0',
      contract: contract ? { id: contract.contractId, fingerprint: contract.metadata?.contractFingerprint } : null,
      platform,
      environment,
      context: contract && (contract as { executionContext?: unknown }).executionContext
        ? (contract as { executionContext: unknown }).executionContext
        : { status: 'missing', reason: 'Contract has no executionContext; external mapping and approval are required.' },
      operations: plan,
    }, null, 2), 'utf8');
  }
